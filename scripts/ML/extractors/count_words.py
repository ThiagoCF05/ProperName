...
def priori_ds(entity, vocabulary):
    ...
    f3 = filter(lambda y: y['bigram'][0] == '*', f2)
...
def syntax_given_w(voc):
    grams = map(lambda x: (x['syntax'], x['word'], x['entity']), voc)
    return dict(nltk.FreqDist(grams))

def run(vocabulary, entities):
    _priori = {}
    ...
    discourse_w = discourse_given_w(vocabulary)
    sentence_w = sentence_given_w(vocabulary)
    entity_w = entity_given_w(vocabulary)
    syntax_w = syntax_given_w(vocabulary)
...
if __name__ == '__main__':
    vocabulary = json.load(open('/roaming/tcastrof/names/stats/vocabulary.json'))
